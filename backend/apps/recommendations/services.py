...
class Diversifier:
    ...
    def apply(
        self, ranked_features: list[ListingFeatures], limit: int, context: RecommendationContext
        ...
        if exploration_candidates:
            ...
            # md5 здесь не криптография, а способ получить одинаковый порядок
            # для одной и той же сессии: подсказка usedforsecurity выключает
            # проверку FIPS и на сам хеш не влияет.
            ...
            exploration_candidates.sort(
                key=lambda f: (
                    hashlib.md5(
                        f"{seed}:{f.listing.id}".encode(), usedforsecurity=False
                    ).hexdigest(),
                    f.total_score,
                ),
                reverse=True,
...
class TasteProfileService:
    ...
    def build_profile(context: RecommendationContext) -> dict[str, Any]:
        ...
        # Get raw events
        events = event_qs.select_related("listing", "listing__district").order_by("-created_at")[
            :500
        ]
        ...
        if context.user:
            ...
            domain_favorites = list(
                Favourite.objects.filter(user=context.user).select_related(
                    "listing", "listing__district"
                )[:100]
            )
            # Add canonical views
            domain_views = list(
                ViewHistory.objects.filter(user=context.user)
                .select_related("listing", "listing__district")
        ...
        for fav in domain_favorites:
            ...
            if listing.price_usd:
                profile["prices_by_kind"].setdefault(listing.kind, []).append(
        ...
        for ev in events:
            ...
            if ev.event_type == InteractionType.SEARCH:
                ...
                half_life_days = (
                    constants.SESSION_HALF_LIFE_MINUTES / 1440.0
                    if is_current_session
                    else constants.DECAY_HALF_LIFE_DAYS
                )
            ...
            if ev.listing:
                if ev.listing.price_usd and weight > 0:
                    profile["prices_by_kind"].setdefault(ev.listing.kind, []).append(
                        float(ev.listing.price_usd)
                    )
