...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        context = RecommendationContext(user=user, session_id="benchmark_session", limit=20)
        ...
        for _ in range(5):
            ...
            # Трогаем поля, чтобы учесть накладные расходы сериализации,
            # даже когда полный сериализатор не используется.
        ...
        for i, t in enumerate(timings):
            self.stdout.write(
                f"Run {i + 1}: Total {t['total']:.2f}ms "
                f"(Rank {t['rank']:.2f}ms, Serialize {t['serialize']:.2f}ms), "
                f"Queries: {t['queries']}"
            )
