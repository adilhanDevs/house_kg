...
def require_terms_version(value: str | None) -> str:
    ...
    предупреждение: регистрация просто не состоится. Какое-то время функция
    подставляла текущую версию сама — согласие записывалось за пользователя,
    который ничего не видел и ничего не принимал.
    """
    if not value:
