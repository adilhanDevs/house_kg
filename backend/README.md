...
в той же базе (`PushOutbox`), брокера в этом пути нет.

Схема доставки:

```
доменная транзакция → строка Notification + строка PushOutbox → commit
  → manage.py run_push_worker → deliver_notification_push(notification_id)
  → настройки → устройства → FCM
```

Брокера нет по двум причинам. Первая — память: прод живёт на 458 МБ, и Redis
с воркером Celery вдвоём не помещаются в остаток до порога безопасности.
Вторая важнее: связка «коммит → постановка в брокер» имеет щель, в которую
проваливается упавший процесс, и уведомление остаётся недоставленным
навсегда. Строка очереди пишется в той же транзакции, что и уведомление, —
либо есть обе, либо нет ни одной.

Воркер забирает пачку строк короткой транзакцией с `select_for_update(
skip_locked=True)`, отпускает её и только потом идёт в сеть: держать
транзакцию открытой на время запроса к Firebase значило бы занимать
соединение к базе на секунды. Строки, зависшие в `processing` после падения
процесса, подбираются заново через 10 минут. Повторы ограничены: 1 мин →
5 мин → 15 мин → 1 час, дальше `failed`. Мёртвые токены гасятся.

Падение FCM или очереди не ломает отправку сообщения и не мешает менять
цену: push — доставка по возможности, а не часть бизнес-события.

Маршрут Celery в конфиге сохранён как необязательный адаптер на будущее;
продакшн его не использует и Redis для push не требует.

Запуск воркера:

```bash
python manage.py run_push_worker          # цикл
python manage.py run_push_worker --once   # один проход
```

### Выключенный режим

`PUSH_ENABLED=0` (по умолчанию) — уведомления создаются и лежат в базе,
отправки нет, файл service-account не открывается и `firebase_admin` не
импортируется. В таком виде прод поднимается без единого секрета на диске.

Строка очереди при этом помечается `skipped_disabled` и обратно в работу не
возвращается. Это намеренно: иначе к моменту появления Firebase накопился бы
месяц старых уведомлений и разом улетел людям. После включения доставляются
только новые события.

### Когда появятся креды Firebase

1. Положить service-account на сервер:

   ```
   /root/house-backend/secrets/firebase-service-account.json
   chmod 600
   ```

   Каталог `backend/secrets/` в `.gitignore`; заглушку вместо настоящего
   файла не создаём — по ней невозможно отличить «ещё не настроено» от
   «настроено неверно».
...
2. В `.env` прода:
   ...
   ```
   PUSH_ENABLED=1
   FCM_CREDENTIALS_FILE=/root/house-backend/secrets/firebase-service-account.json
   ```

3. Перезапустить gunicorn и воркер push-очереди.

4. Проверить на себе, не трогая живые события:

   ```
   python manage.py send_test_push --user-id <id>
   ```

   Команда требует явный `--user-id` и активное устройство: без них она
   отказывается работать, а не отправляет «кому-нибудь». При `PUSH_ENABLED=0`
   она ничего не создаёт и честно об этом пишет.

Ни токен устройства целиком, ни содержимое кредов в логи не попадают —
пишутся только счётчики, идентификаторы и классы ошибок.
