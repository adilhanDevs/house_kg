...
> **Транспорт изменён после замеров на проде (2026-09-03).** Redis и воркер
> Celery в этот документ заложены как транспорт доставки, но на боевом сервере
> они не поместились: 458 МБ памяти, свободно ~155, и связка Redis + Celery
> оставляла бы меньше порога безопасности в 64 МБ. К тому же в журнале ядра
> уже накопилось 44 OOM-kill gunicorn.
>
> Поэтому доставка идёт через очередь в PostgreSQL (`PushOutbox`) и лёгкий
> процесс `manage.py run_push_worker`. Побочно это устранило щель между
> коммитом и постановкой в брокер: строка очереди пишется в той же транзакции,
> что и уведомление. Redis на проде не установлен, Celery для push не нужен.
>
> Разделы про Redis, очередь `push` и Celery-воркер ниже сохранены как история
> решения; действующее описание транспорта — в `backend/README.md`.
