...
from apps.catalog.enums import Currency, ListingStatus, PropertyKind
from apps.catalog.models import City, District, Listing
...
def test_new_user_feed_is_diverse_and_fresh():
    ...
    city, _ = City.objects.get_or_create(
        slug="bishkek", defaults={"name": "Bishkek", "is_active": True}
    )
...
def test_reels_watch_improves_score():
    ...
    RecommendationEvent.objects.create(
        user=user,
        session_id="sess1",
        event_type=InteractionType.REEL_WATCH,
        listing=listing_asanbay,
        context={"watch_ratio": 0.95},
    )
...
def test_immediate_skip_negative_penalty():
    ...
    RecommendationEvent.objects.create(
        user=user, session_id="sess1", event_type=InteractionType.REEL_SKIP, listing=listing_skipped
    )
