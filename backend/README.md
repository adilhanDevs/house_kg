...
Источник правды — таблица `Notification` в PostgreSQL. Redis здесь только
брокер: очередь можно потерять целиком, уведомления в базе останутся.

Схема доставки:

```
доменная транзакция → строка Notification → commit
  → transaction.on_commit → deliver_notification_push(notification_id)
  → очередь push → воркер → FCM
```

Задача принимает только идентификатор и маршрутизируется в очередь `push`
конфигом Celery, а не вызовом, поэтому `.delay()` из любого места попадёт
именно туда. Падение брокера или FCM не ломает отправку сообщения и не мешает
менять цену: push — доставка по возможности, а не часть транзакции.
